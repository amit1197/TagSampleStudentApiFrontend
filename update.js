...
async function fetchStudentData() {
    ...
    try {
        let response = await fetch(`http://localhost:5041/api/Student/GetbyID?student_Id=${studentId}`);
        if (!response.ok) {
...
async function updateStudentData() {
    ...
    try {
        let response = await fetch(`http://localhost:5041/api/Student/Update?student_Id=${studentData.studentId}`, {
            method: 'PUT', 
            headers: {
